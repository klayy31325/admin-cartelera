...
export function ExcelImportForm() {
  ...
  async function sendRequest(preview: boolean) {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/api/trabajos/import${preview ? "?preview=true" : ""}`,
  ...
  return (
    <div className="bg-muted/20 backdrop-blur-sm p-6 md:p-8 rounded-[2rem] border border-border shadow-soft space-y-8 max-w-4xl mx-auto">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div
          ...
          className={cn(
            "relative border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-300 cursor-pointer flex flex-col items-center justify-center gap-4 min-h-[200px]",
            isDragging ? "border-brand bg-brand/10" : "border-zinc-700 hover:border-brand/50 hover:bg-zinc-800/30",
      ...
      {result && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-zinc-800">
          <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4 text-center">
            <p className="text-3xl font-black text-green-400">{result.insertados}</p>
            <p className="text-[10px] font-black uppercase tracking-widest text-green-500/60 mt-1">Trabajos</p>
          </div>
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 text-center">
            <p className="text-3xl font-black text-blue-400">{result.velocidad_insertada}</p>
          ...
          <div className="bg-purple-500/10 border border-purple-500/20 rounded-2xl p-4 text-center">
            <p className="text-3xl font-black text-purple-400">{result.desperdicio_insertado}</p>
            <p className="text-[10px] font-black uppercase tracking-widest text-purple-500/60 mt-1">Desperdicios</p>
          </div>
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 text-center">
            <p className="text-3xl font-black text-amber-400">{result.duplicados}</p>
            <p className="text-[10px] font-black uppercase tracking-widest text-amber-500/60 mt-1">Duplicados</p>
          </div>
          {result.errores.length > 0 && (
            <div className="col-span-2 md:col-span-4 bg-red-500/10 border-2 border-red-500/30 rounded-2xl p-6 space-y-3 animate-in zoom-in-95 duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                {result.errores.map((e, i) => (
                  <div key={i} className="flex gap-3 items-start bg-red-500/5 p-2 rounded-lg border border-red-500/10">
                    ...
                    <p className="text-[11px] text-red-400 font-medium">{e.razon}</p>
                  </div>
                ))}
              </div>
